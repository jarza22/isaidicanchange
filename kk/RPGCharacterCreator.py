...
class CharacterGenerator:
    ...
    def __init__(self,dice):
        ...
        self.skills = {skill: 5 for skill in self.governingSpecial.keys()}
        self.attributes = {
            "hitpoints": 1,
            "actionpoints": 1,
            "carryweight": 1,
    ...
    def buildCharacter(self):
        ...
        assignableGenders = ["M", "F", "O"]
        while True:
            inputtedGender = input("Please enter your gender (M/F/O) ").upper()
            if inputtedGender in assignableGenders:
                self.gender = inputtedGender
                if self.gender == "M":
                ...
                print("Your answer is invalid. Please enter one of these options. (M/F/O)")

        for skill, special_stat in self.governingSpecial.items():
            self.skills[skill] += 2 * self.special[special_stat] + self.special["luck"] // 2

        self.attributes["hitpoints"] = sum(self.dice.roll("D100") for _ in range(self.special["endurance"])) + sum(self.dice.roll("D20") for _ in range(self.special["luck"]))
        self.attributes["actionpoints"] = sum(self.dice.roll("D10") for _ in range(self.special["agility"]))
        self.attributes["carryweight"] = 100 + sum(self.dice.roll("D20") for _ in range(self.special["strength"]))
        self.attributes["radiationresist"] = sum(self.dice.roll("D2") for _ in range(self.special["endurance"]))
        self.attributes["compassrange"] = 100 + sum(self.dice.roll("D6") for _ in range(self.special["perception"])) + sum(self.dice.roll("D4") for _ in range(self.special["intelligence"]))
        self.attributes["movespeed"] = sum(self.dice.roll("D50") for _ in range(self.special["agility"])) // 2

    def __str__(self):
        character_info = f"Name: {self.name}\nAge: {self.age}\nGender: {self.gender}\n"
        character_info += "SPECIALs:\n"
        for stat, value in self.special.items():
            character_info += f"{stat.capitalize()}: {value}\n"
        character_info += "Skills:\n"
        for skill, value in self.skills.items():
            character_info += f"{skill.capitalize()}: {value}\n"
        character_info += "Attributes:\n"
        for attr, value in self.attributes.items():
            character_info += f"{attr.capitalize()}: {value}\n"
